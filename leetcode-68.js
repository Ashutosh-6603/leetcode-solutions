...
Note: The input array words contains at least one word.
...
function fullJustify(words, maxWidth) {
  ...
  for (const word of words) {
    if (currentLineLength + word.length + currentLine.length > maxWidth) {
      ...
      const spacesBetweenWords = currentLine.length - 1 || 1;
      const evenSpaces = Math.floor(totalSpaces / spacesBetweenWords);
      ...
      for (let i = 0; i < currentLine.length; i++) {
        ...
        if (i < currentLine.length - 1) {
          line += " ".repeat(evenSpaces + (i < extraSpaces ? 1 : 0));
        }
      }
  ...
  const lastLine =
    currentLine.join(" ") +
    " ".repeat(maxWidth - currentLineLength - currentLine.length + 1);
